...
def main():
    ...
    uri = create_token_uri(dev, token_id)
...
def create_token_uri(addr, token_id):
    addr_str = str(addr)
    t_id_str = str(token_id)
    img = execute(addr_str + t_id_str)
    name = "lovely-robot-" + t_id_str + ".png"
    ipfs_hash = upload_binary(img, name)
    metadata_template["name"] = "Lovely Robot #" + t_id_str
    metadata_template["description"] = "A unique robot generated by amazing RoboHash.org especially for YOU."
